...
function init(){
    ...
    document.addEventListener('keypress', function (e) {
        ...
        // Si el carro esta en tour no dejar usar las direccionales
        if(car.tourMode)
            return;

        // L
        if(e.which == 108){
            $("#btnLeftDirectional").trigger("click");
        }
        // R
        if(e.which == 114){
            $("#btnRightDirectional").trigger("click");
        }
    });
    ...
    // Nuevo tour
    let newTour = new tour(car);

    // Push: agregar un nuevo elemento al array de tours del carro
    car.tours.push(newTour);
    setTimeout(() => {
        car.tours[0].startTour();    
    }, 3000);
